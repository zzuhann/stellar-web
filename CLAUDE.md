...
### 🎭 藝人管理 API
```
GET    /api/artists              - 取得所有已審核藝人
POST   /api/artists              - 新增藝人（需登入）
PUT    /api/artists/:id/approve  - 審核藝人（需管理員）
PUT    /api/artists/:id/reject   - 拒絕藝人（需管理員）
DELETE /api/artists/:id          - 軟刪除藝人（需管理員）
```

### ☕ 活動管理 API
```
GET    /api/events                    - 取得所有已審核活動
GET    /api/events/:id                - 取得單一活動詳情
GET    /api/events/search             - 搜尋活動（支援藝人名稱、地點）
POST   /api/events                    - 新增活動（需登入）
PUT    /api/events/:id/approve        - 審核活動（需管理員）
PUT    /api/events/:id/reject         - 拒絕活動（需管理員）
DELETE /api/events/:id               - 軟刪除活動（需管理員）
```
